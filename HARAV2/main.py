...
from src.pose_estimator_V2 import PoseEstimator3D
from src.kalman_filter import KalmanFilterTracker3D
from src.skeleton_visualizer import SkeletonVisualizer
from src.kalman_filter import KeypointPreprocess
from src.com_calculator import COMCalculator
from model.DeepLSTM import HAPmodel
...
class InferenceVideo:
    def __init__(self, roi_padding, roi_ratio, tracker_type=False,
                 binary=False, checkpoint_path=None, device='cpu'):
        """
        3D 포즈 추적 시스템 초기화
        Args:
        ...
        """
        self.pose_estimator = PoseEstimator3D(min_detection_confidence = 0.8,
                                              min_tracking_confidence = 0.5, 
                                              tracker_type=tracker_type, 
                                              roi_padding=roi_padding, 
                                              roi_ratio=roi_ratio)
        
        self.net = HAPmodel(device, binary, checkpoint_path)
    ...
    def process_frame(self, frame, loop_num, launch_sign):
        try:
            # 1. 포즈 추정
            sequence_data = []
            landmarks, processed_frame = self.pose_estimator.estimate_pose(frame, loop_num, launch_sign)
    ...
    def run(self, input_source):        
        ...
        """
        try:
            sequence_tensor, reset_button_pressed, launch_sign = [], False, True
            output_video, current_frame, loop_num = None, 10, 0
            status = next_status = ['None', (0, 0, 255)]
            recent_predictions, stable_prediction = [], None
            stable_counter, STABILITY_THRESHOLD = 0, 5
            ...
            first_sequence_vector, _ = self.process_frame(first_frame, loop_num, launch_sign)
            
            if first_sequence_vector:
                sequence_tensor.append(first_sequence_vector)
                launch_sign = False
            else:
                logger.error("Failed to read first frame")
            ...
            while True:
                ...
                if not ret:
                    self._handle_end_of_video(input_source, loop_num+10)
                    break
                ...
                if len(sequence_tensor) >= current_frame:
                    sequence_tensor = sequence_tensor[stride:]
                
                sequence_vector, visual_frame = self.process_frame(frame, loop_num, launch_sign)
                    
                if sequence_vector:
                    reset_button_pressed, launch_sign = False, False
                        ...
                        if input_source == 0:
                            if self.binary:
                                if predicted_class == 0:
                                    next_status = ['Action: Stand', (0, 255, 0)]
                                elif predicted_class == 1:
                                    next_status = ['Action: During Walking', (255, 0, 0)]
                            else:
                                if predicted_class in [0, 1]:
                                    next_status = ['Action: Stand', (0, 255, 0)]
                                elif predicted_class in [2, 3]:
                                    next_status = ['Action: During Walking', (0, 0, 255)]
                        else:
                            if len(recent_predictions) == STABILITY_THRESHOLD and all(p == recent_predictions[0] for p in recent_predictions):
                                if self.binary:
                                    if predicted_class == 0:
                                        next_status = ['Action: Stand', (0, 255, 0)]
                                    elif predicted_class == 1:
                    ...
                    cv2.putText(visual_frame, status[0], (10, 50), cv2.FONT_HERSHEY_COMPLEX, 1.2, (0,0,0), 4, cv2.LINE_AA)
                    cv2.putText(visual_frame, status[0], (10, 50), cv2.FONT_HERSHEY_COMPLEX, 1.2, status[1], 2, cv2.LINE_AA)
                    ...
                    cv2.putText(visual_frame, 'No Target', (10, 50), cv2.FONT_HERSHEY_COMPLEX, 1.2, (0,0,0), 4, cv2.LINE_AA)
                    cv2.putText(visual_frame, 'No Target', (10, 50), cv2.FONT_HERSHEY_COMPLEX, 1.2, (0,0,255), 2, cv2.LINE_AA)
...
def main():
    ...
    parser.add_argument('-p', '--padding', type=float, default=40,
                        help='ROI 여유 공간 (픽셀). 기본값: 40')
    parser.add_argument('-r', '--roi', type=float, default=0.8,
                        help='ROI 비율 (0.0~1.0). 기본값: 0.8')

    # 모델 parameter 설정
    parser.add_argument('-b', '--binary', action='store_true',
                    help='이진 분류 사용 여부 (붙이면 True, 안 붙이면 False)')
    parser.add_argument('-t', '--tracker-type', action='store_true',
                    help='yolo track 모델을 사용할지 여부 (붙이면 True, 안 붙이면 False)')

    # checkpoint 불러오기
    parser.add_argument('--ckp', action='store_true',
                        help='커스텀 HAP 모델 체크포인트 활성화 (기본값: 비활성화)')
    parser.add_argument('--ckp-path', type=str,
                        help='커스텀 HAP 모델 체크포인트 디렉터리 경로 (--ckp가 활성화된 경우에만 사용)')
    ...
    # 커스텀 체크포인트 처리
    if args.ckp:
        if not args.ckp_path:
            logger.error("--ckp 옵션이 활성화되었지만 --ckp-path 경로가 지정되지 않았습니다.")
            sys.exit(1)
        elif not os.path.exists(args.ckp_path):
            logger.error(f"Error: 지정된 체크포인트 경로가 존재하지 않습니다: {args.ckp_path}")
            sys.exit(1)
        
    # 시스템 초기화 및 실행
    pose_tracking = InferenceVideo(
