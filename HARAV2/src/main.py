...
from pose_estimator_V2 import PoseEstimator3D
...
class InferenceVideo:
    def __init__(self, roi_padding, roi_ratio, tracker_type, 
                 use_tft=False, tft_checkpoint_dir=None, tft_sequence_length=20, 
                 tft_binary_mode=False, csv_export=True, csv_filename='keypoints_export_normalized.csv', 
                 device=device, point_remain='all'):
        """
        3D 포즈 추적 시스템 초기화
        Args:
        ...
        self.pose_estimator = PoseEstimator3D(min_detection_confidence = 0.8,
                                              ...
                                              tracker_config='../mmtracking/configs/mot/deepsort/sort_faster-rcnn_fpn_4e_mot17-private.py')
    ...
    def run(self, input_source):        
        ...
        """
        try:
            sequence_tensor, no_target_cycle, launch_sign = [], 0, True
            ...
            first_sequence_vector, processed_first_frame = self.process_frame(first_frame, loop_num, launch_sign)
                ...
                if sequence_vector:
                    ...
                    if len(sequence_tensor) == current_frame:
                        ...
                        with torch.no_grad():
                            ...
                            if outs.item() == 0 : next_status = ['Stand', (0, 255, 0)]
                            elif outs.item() == 1 : next_status = ['Start Walking', (0, 0, 255)]
                            elif outs.item() == 2 : next_status = ['During Walking', (255, 0, 0)]
                            else : next_status = ['Finish Walking', (0, 0, 255)]
                        
                    cv2.putText(visual_frame, status[0], (0, 50), cv2.FONT_HERSHEY_COMPLEX, 1.5, status[1], 2)
                    ...
                    cv2.putText(visual_frame, 'No Target', (0, 50), cv2.FONT_HERSHEY_COMPLEX, 1.5, (0, 0, 255), 2)
...
def main():
    ...
    parser.add_argument('-t', '--tracker', type=str, default='base',
                        help='Tracker 설정. 기본값: base')
    parser.add_argument('-k', '--kepoints', type=str, default='all',
                        help='모델 입력 Keypoint 설정. 기본값: all')
