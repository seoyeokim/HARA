...
import numpy as np
from roi_tracker import Tracker, mmTracker

class PoseEstimator3D:
    def __init__(self,
                 min_detection_confidence=0.5,
                 min_tracking_confidence=0.5,
                 tracker_type="roi",  # or "mmtrack" or None
                 tracker_config=None,
                 tracker_ckpt=None,
                 device="cuda",
                 roi_padding=40,
        ...
        if tracker_type == "base":
            self.tracker = Tracker(roi_padding, roi_ratio)
        elif tracker_type == "mmtrack":
            self.tracker = mmTracker(tracker_config, device, roi_padding, roi_ratio, 0.1, 0.1, tracker_ckpt)
    ...
    def estimate_pose(self, frame, frame_num=None, launch_sign=False):
        ...
        # ROI 계산
        if self.tracker_type == "mmtrack" and self.tracker:
            try:
                if (frame_num % 5) == 0:
                    new_roi, self.target_id = self.tracker.computed_roi(frame, int(frame_num/5),
                                                                        launch_sign,
        ...
        # 포즈 추정 수행
        if self.roi_bbox or (self.tracker_type == "base"):
            try:
            ...
            if results.pose_landmarks:
                ...
                elif self.tracker_type == "base" and self.tracker:
                    try:
                ...
                if self.tracker_type == "base" and self.tracker:
                    self.roi_bbox = self.tracker._default_roi(frame)
