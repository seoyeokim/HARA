...
class mmTracker:
    def __init__(self, config, device, roi_padding=20, choice_area=0.7, iou_treshold=0.1, adjust_rate=0.1, checkpoint=None):
    ...
    def computed_roi(self, frame, frame_num, target_reset, target_id):
        height, width = frame.shape[:2]
        
        result = inference_mot(self.model, frame, frame_id=frame_num)
        bboxs = result['track_bboxes'][0]
        
        if target_reset:
            target_id = self._choice_target(height, width, bboxs)

        if target_id != None:
            target_bbox = bboxs[bboxs[:, 0] == target_id]
