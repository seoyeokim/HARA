...
from kalman_filter import TFTKeypointPreprocess
...
class DatasetCreater:
    def __init__(self, roi_ratio=0.8, roi_padding=80, data_path=None, save_path=None, class_type='nomal', point_remain='all'):
        ...
        self.keypoint_preprocessing = TFTKeypointPreprocess()
    ...
    def run(self):
        ...
        for datas in self.files:
            ...
            self.pose_estimator = PoseEstimator3D(roi_ratio=1, roi_padding=80)
            self.kalman_tracker = KalmanFilterTracker3D()
            self.com_calculator = COMCalculator()
            self.keypoint_preprocessing = TFTKeypointPreprocess()
            self.kalman_tracker = KalmanFilterTracker3D() # 칼만 필터 재초기화
            self.stable_com_pos = None
...
if __name__ == "__main__":
    ...
    pose_tracking = DatasetCreater(roi_ratio=1.0, data_path=args.load_path, save_path=args.save_path, class_type=args.class_type, point_remain=args.point_remain)
    pose_tracking.run()
